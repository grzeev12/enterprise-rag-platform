...
export default async function middleware(request: NextRequest) {
  ...
  if (isLoginPage && hasToken) {
    ...
    logMiddlewareDecision({ pathname, hasToken, isLoginPage, redirectTarget: dashboardUrl.pathname });
  ...
  if (isProtected && !hasToken) {
    ...
    logMiddlewareDecision({ pathname, hasToken, isLoginPage, redirectTarget: `${loginUrl.pathname}${loginUrl.search}` });
    return withHeaders(NextResponse.redirect(loginUrl), requestId, correlationId);
  }

  logMiddlewareDecision({ pathname, hasToken, isLoginPage, redirectTarget: null });
